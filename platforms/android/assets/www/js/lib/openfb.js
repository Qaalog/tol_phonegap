...
var openFB = (function () {  
    ...
    function login(callback, options) {
        ...
        loginWindowLoad = function(event) {
          console.log('dom loaded');
          if (loginWindow.parent == null) {
            clearInterval(intervalId);
          }
         
          try {
            var userToken = /access_token=(.+)&/.exec(loginWindow.location.hash)[1];
            oauthCallback(loginWindow.location.hash);
            if (userToken && userToken !== null) {
              loginWindow.close();
              loginWindow.removeEventListener('DOMContentLoaded', loginWindow_loadStartHandler);
              clearInterval(intervalId);
            }
          } catch(e) {
            console.log('other try');
          }
        };
        ...
        if (!runningInCordova) {
          //loginWindow.addEventListener('DOMContentLoaded', loginWindow_loadBrowserHandler);
          //loginWindow.onload = loginWindow_loadBrowserHandler;
    ...
    function logout(callback) {
        ...
        if (token) {
            logoutWindow = window.open(logoutURL + '?access_token=' + token, '_blank', 'location=no,clearcache=yes');
            //if (runningInCordova) {
                setTimeout(function() {
                  logoutWindow.close();
                }, 1000);
                var http = new XMLHttpRequest();
                http.open('POST',logoutURL,true);
                http.onreadystatechange = function() {
                  if (http.readyState === 4) {
                    //console.log(http.responseText);
                  }
                };
                http.send('access_token=' + token);
           // }
        }
    ...
    function api(obj) {
        ...
        params['access_token'] = tokenStore.fbAccessToken; //|| token;
