tol.controller('givePoints',['$scope','page','network','header','userService','imageUpload','device','dialog','$timeout','lightbox','pager',
  'analytics','$timeout','feed','$sce',
  function($scope, page, network,header,userService,imageUpload,device,dialog,$timeout,lightbox,pager,analytics, $timeout,feed,$sce){
  ...
  $scope.preScroll = function() {
    ...
    if (container && header && textArea) {
      ...
      var conteinerPaddingTop = parseInt(getComputedStyle(container).paddingTop);
  ...
  var validate = function() {
    if ($scope.category !== undefined && $scope.mark !== false && $scope.mark !== undefined) {
    ...
    network.post('product/getPoints/',{id: $scope.product.id},function(result, response) {
      if (result) {
        ...
        if (!$scope.categoryConfig[$scope.categoryMark] || !$scope.categoryConfig[$scope.categoryMark]['point_values']) {
          page.goBack();
          $timeout(function() {
            dialog.create(dialog.INFO, 'Oppps', 'Points categiry is empty, please contact with administrator', 'Ok').show();
          }, 500);
          return false;
        }
  ...
  function givePoints(postId, isPictureEmpty) {
    ...
    if ($scope.params.pointsForPost) {
      ...
      if ($scope.isReinforce) {
        updateReason = userService.getAuthProduct().name.trim()+ ' reinforced this';
      }
    ...
    network.put('points_given/',data,function(result, response) {
      if (result) {
        ...
        if (isPictureEmpty) {
          ...
          if (!$scope.isCreatePost || $scope.isQuotePost) {
            console.log('test1');
            network.pagerUpdate();
          } else {
            console.log('test2');
  ...
  function giveMultiPoints(postId, isPictureEmpty) {
    ...
    if (!postId) {
      var updateReason = '';
      if ($scope.recognizeList.length < 3) {
