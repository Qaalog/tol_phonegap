tol.controller('givePoints', ['$scope', 'page', 'network', 'header', 'userService', 'imageUpload', 'device', 'dialog', '$timeout', 'lightbox', 'pager',
    ...
    function ($scope, page, network, header, userService, imageUpload, device, dialog, $timeout, lightbox, pager, analytics, $timeout, feed, $sce, $rootScope) {
        ...
        page.onShow(settings, function (params) {
            ...
            $scope.AUTO_POSTS = feed.AUTO_POSTS;
        ...
        function reinforce(params) {//TODO Refactor it to work with ID
        ...
        $scope.addMore = function () {
            ...
            //-----------------------------------------------
            var data = {
                recognizeList: $scope.recognizeList
                , savedData: {
        ...
        function givePoints(postId, isPictureEmpty) {
            if(typeof $scope.point == 'undefined' || typeof $scope.pointsAlias == 'undefined'
                || typeof $scope.category == 'undefined' || typeof $scope.categoryConfig[$scope.category].id == 'undefined'
                || $scope.point == null || $scope.pointsAlias == null
                || $scope.category == null || $scope.categoryConfig[$scope.category].id == null
                || $scope.point === false || $scope.pointsAlias === false
                || $scope.category === false || $scope.categoryConfig[$scope.category].id === false
            ){
                page.hideLoader();
                dialog.create(dialog.INFO,'Oooops!',
                    'Something went wrong...<br>Please try again!','OK','', function() {
                    }).show();
                return;
            }
            var data = {
            ...
            network.put('points_given/', data, function (result, response) {
                if (result) {
                    ...
                    if (isPictureEmpty) {
                        network.post('post/' + data['post_id'], {deleted:null}, function (result, response) {
                            network.post('post/updatePointsPostImage', {post_id: data['post_id']}, function (result) {
                                $scope.savedData = {};
                                $scope.recognizeList = [];
                                $rootScope.$broadcast('clearSavedDataEvent');
                                console.log('recopost with picture saved successfully and marked as undeleted to show');
                                ...
                                $timeout(function () {
                                    analytics.trackCustomMetric(analytics.POINTS_GIVEN, pointsToAnalytics.toString(), function () {
                                        analytics.timeEnd('Post speed', function () {
                                            analytics.trackCustomMetric(analytics.POINTS_GIVEN, 0, function () {
                                                analytics.trackCustomDimension(analytics.POST_TYPE, 'Recognition post', function () {
                                                    analytics.trackCustomMetric(analytics.POST_MADE, 1, function () {
                                                        analytics.trackEvent(['DO IT button on post'
                                                            , 'Recognition post'
                                                            , 1
                                                        ], false, function () {
                                                            analytics.trackCustomDimension(analytics.POST_TYPE, '');
                                                            analytics.trackCustomMetric(analytics.POST_MADE, 0);
                                                        });
                                                    });
                                                });
                                            });
...
//    });
        ...
        $scope.showWhatIsThis = function () {
//            feed.getCatalog(function(selectedCatalog) {
            var selectedCatalog = userService.getCatalogSelected();
            if (selectedCatalog) {
                if (selectedCatalog.parameters && selectedCatalog.parameters.recognition_description) {
                    dialog.create(dialog.INFO, 'Recognition', selectedCatalog.parameters.recognition_description, 'OK', '').show();
                }
            }
  //          });
        };
